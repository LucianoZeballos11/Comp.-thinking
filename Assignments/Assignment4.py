...
try:
    ...
    if 0 < score < 100: 
        ...
        print(f"Grade is: {grade}") #program output
    else:
        print("Error, please enter a numeric input between 0 and 100") #program output if the value is below 0 or grader than 100
except:
...
try: 
    ...
    if hours <= 40:
        print(f"Pay: {salary}") #program output if hours is below to 40 hours
    else: 
...
while True: #infine loop
    try: 
        ...
        num = int(InpNum) #define the input as an integrer
        if num < 0:
            ...
            print(f"Number divisible by 3 among numbers from 1 to {num}: {result}") #program output of the final result
    except:
